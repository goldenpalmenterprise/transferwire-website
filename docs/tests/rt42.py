...
with sync_playwright() as pw:
    ...
    try:
        ...
        sheet = p.locator("div", has_text="Profil speichern").last
        p.locator("input[placeholder]").filter(has=p.locator(":scope")).first.fill("Sang-Yoon Kang")
        p.locator("select").last.select_option("ZM")
        nums = p.locator("input[type=number], input[inputmode=numeric]")
        if nums.count() >= 2: nums.nth(0).fill("24"); nums.nth(1).fill("24")
